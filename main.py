...
def main():
    ...
    plt.plot(all_data.Date.values,
            all_data.Gold_INR.values/ all_data.Gold_INR.values[0], label="Gold_INR")
    plt.plot(all_data.Date.values,
            all_data.Ex.values/ all_data.Ex.values[0], label="Exchange")
    plt.plot(all_data.Date.values,
            all_data.Gold_USD.values/ all_data.Gold_USD.values[0], label="Gold_USD")

    plt.xticks([all_data.Date[x] for x in range(all_data.shape[0]) if x % 15 == 0])
